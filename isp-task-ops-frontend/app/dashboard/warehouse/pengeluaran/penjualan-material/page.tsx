...
export default function PenjualanMaterialPage() {
  const items = useStockStore((state) => state.items);
  const [materialId, setMaterialId] = useState(items[0]?.id ?? "");
  const [qty, setQty] = useState(1);
  const [toko, setToko] = useState("Toko Mitra Jaringan");
  const [alamat, setAlamat] = useState("Jl. Kemang Raya No. 12");
  const [nomorTelepon, setNomorTelepon] = useState("082233445566");
  const [info, setInfo] = useState("");

  return (
    <div className="space-y-4">
      <h1 className="text-2xl font-bold">Pengeluaran Barang - Penjualan Material</h1>
      <Card title="Form Penjualan Material">
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
          ...
          <div>
            <label className="mb-1 block text-sm font-semibold">Toko</label>
            <input className="tap-target w-full rounded-md border border-gray-300 bg-white" value={toko} onChange={(e) => setToko(e.target.value)} />
          </div>
          <div>
          ...
          <div className="md:col-span-2 flex flex-wrap gap-2">
            <Button type="button" onClick={() => setInfo("Barang berhasil dikirim (mock).")}>Kirim Barang</Button>
            <Button type="button" variant="secondary" onClick={() => setInfo("Laporan penjualan siap dicetak (mock).")}>Print Laporan</Button>
          </div>
        </div>
